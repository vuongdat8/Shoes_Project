    ...
    public class LoginActivity extends AppCompatActivity {
        ...
        private void handleGoogleIntent(Intent data) {
            Log.d(TAG, "handleGoogleIntent() called");
        ...
        /* ---------------- Firebase chung ----------------------------- */
        private void firebaseSignIn(AuthCredential cred) {
            Log.d(TAG, "firebaseSignIn() started");
            mAuth.signInWithCredential(cred)
                    .addOnCompleteListener(this, task -> {
                        if (task.isSuccessful()) {
                            ...
                            Log.e(TAG, "firebaseSignIn() FAILED", e);
                            Toast.makeText(this,
                                    e != null ? e.getMessage() : "Đăng nhập thất bại",
        ...
        private void saveFirebaseSession() {
            ...
            // ✨ Nếu email này nằm trong danh sách admin
            if (email != null && (
                    email.equals("ninjaanhem@gmail.com") ||
                            email.equals("zzakonlovez@example.com")
            )) {
                isAdmin = true;
            }
            ...
            getSharedPreferences(PREF_AUTH, MODE_PRIVATE)
                    ...
                    .putInt("userId", 0)
                    .putBoolean("role", isAdmin)
                    .apply();

            Log.d(TAG, "Firebase user email: " + email + ", isAdmin=" + isAdmin);
        }
...
//        }
        private void goHome() {
            SharedPreferences prefs = getSharedPreferences(PREF_AUTH, MODE_PRIVATE);
            boolean isAdmin = prefs.getBoolean("role", false);
            String email = prefs.getString("email", "unknown");

            Log.d(TAG, "goHome(): email = " + email + ", isAdmin = " + isAdmin);
